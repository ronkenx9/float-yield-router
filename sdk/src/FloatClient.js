...
export class FloatClient {
    ...
    recallConfirmationTimeoutMs;
    enableRecallReplacement;
    txHistory = [];
    recallHistory = [];
    constructor(config) {
    ...
    async gatewayRecall(params) {
        ...
        const helperAddressToBytes32 = (addr) => ("0x" + addr.toLowerCase().replace(/^0x/, "").padStart(64, "0"));
        const burnIntent = {
    ...
    wrapPayment(paymentExecutor) {
        return async (amount, recipient, options) => {
            ...
            if (liquidBalance < amount) {
                ...
                if (parkedBalance < deficit) {
                    throw new Error(`[FLOAT] Insufficient funds: Liquid ($${liquidBalance.toFixed(2)}) + Parked ` +
                ...
                if (this.recallHistory.length >= this.maxRecallFrequencyPerHour && !force) {
                    throw new Error(`[FLOAT] Recall rate limit exceeded (${this.recallHistory.length}/${this.maxRecallFrequencyPerHour} in last hour). ` +
                        `Pass { force: true } from an EMERGENCY context to override, or increase maxRecallFrequencyPerHour.`);
                }
                if (this.recallHistory.length >= this.maxRecallFrequencyPerHour && force) {
                    console.warn(`[FLOAT] Recall rate limit overridden by force=true. Inspect upstream logic — ` +
                ...
                if (liquidAfter + 1e-6 < amount) {
                    throw new Error(`[FLOAT] Recall confirmed onchain but wallet balance still reports ` +
