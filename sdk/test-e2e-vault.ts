...
async function readDeposits(vaultAddress: string, walletAddress: string): Promise<number> {
  const provider = new ethers.JsonRpcProvider(ARC_RPC);
  const vault = new Contract(vaultAddress, VAULT_ABI, provider) as any;
  const raw = await vault.deposits(walletAddress) as bigint;
...
async function main() {
  ...
  if (liquidBefore < needed) {
    throw new Error(
      `Wallet A needs ≥ ${needed} USDC liquid (have ${liquidBefore}). ` +
  ...
  if (vaultAfterPark < vaultBefore + PARK_AMOUNT - 0.001) {
    throw new Error(
      `Vault deposits did not increase by expected amount. ` +
  ...
  if (liquidBeforeRecall < PAYMENT_AMOUNT) {
    ...
    console.log(
      `→ Liquid (${liquidBeforeRecall}) ≥ payment (${PAYMENT_AMOUNT}). ` +
